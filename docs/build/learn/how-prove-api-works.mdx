...
Polymer reads chain state and events from a deliberately diverse, redundant set of upstreams — RPC endpoints from different providers, data straight from a chain's sequencer, and trusted execution environments (TEEs). No single source is privileged or required; the model is source-agnostic.
...
`validateEvent` verifies **Polymer's signature** over the attestation; it does not re-verify the source chain's consensus on-chain. An attestation's security rests on the m-of-n agreement behind it — a deliberate trade, and the reason verification costs a fraction of a cent and works identically on every supported chain.

Anyone holding a valid proof can submit it, so confirming that a decoded event is one your application should act on — the source chain, the emitting contract, the event signature, and replay protection — is your contract's responsibility. [Prover Integration](<../get started/prove-api-V2/decodingProverReturn.mdx>) covers each of those with code.
