...
Before an on-chain message can be sent, the "Application relayer" or "solver" must first query the Prove API.
1. **Request Proof:** Use the `polymer_requestProof` method to request a proof, which returns a `jobID`.
2. **Poll for Proof:** Use the `polymer_queryProof` method with the `jobID` to poll for proof responses.


Find info [here](https://docs.polymerlabs.org/docs/build/start/) to:
- Access the functions with POST requests to API endpoint
